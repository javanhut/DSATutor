    ...
    pass`,
			...
			Animations: []Storyboard{
				...
				},
				{
					...
					Goal:  "Understand asymptotic analysis, visualize complexity curves, and compare time vs space tradeoffs.",
					MemoryTips: []string{
						"Big O focuses on GROWTH RATE, not exact speed - ignore constants",
						"O(1) is CONSTANT: same time regardless of input size",
						"O(log n) is LOGARITHMIC: each step halves the remaining work",
						"O(n) is LINEAR: time grows directly with input size",
						"O(n log n) is LINEARITHMIC: divide-and-conquer algorithms like merge sort",
						"O(n^2) is QUADRATIC: nested loops over the same data - gets slow fast!",
						"Space complexity matters too - trading memory for speed is common",
					},
					Steps: []StoryboardStep{
						{Cue: "intro-bigo", Narration: "Big O describes how algorithms scale. It tells us which algorithm wins as data grows, ignoring constant factors.", VisualHint: "Asymptotic intro panel", Duration: Duration(3 * time.Second), CodeRef: "concepts: asymptotic notation"},
						{Cue: "plot-axes", Narration: "The graph plots input size (n) against operations. Watch how different complexities diverge.", VisualHint: "Axes: n vs operations", Duration: Duration(2 * time.Second), CodeRef: "runtime_shapes: setup plot"},
						{Cue: "draw-constant", Narration: "O(1) is constant - a flat line. Array lookup, hash access: same speed for 10 or 10 million items.", VisualHint: "Flat green line", Duration: Duration(3 * time.Second), CodeRef: "arr[index] // O(1)"},
						{Cue: "draw-log", Narration: "O(log n) grows slowly. Binary search on a billion items takes only 30 steps because each step halves the data.", VisualHint: "Gentle blue curve", Duration: Duration(3 * time.Second), CodeRef: "while (lo <= hi) mid = (lo+hi)/2"},
						{Cue: "draw-linear", Narration: "O(n) is linear - a straight line. Scanning an array: 1000 items = 1000 operations.", VisualHint: "Orange diagonal line", Duration: Duration(3 * time.Second), CodeRef: "for item in items: process(item)"},
						{Cue: "draw-nlogn", Narration: "O(n log n) is linearithmic - slightly above linear. Efficient sorting like merge sort lives here.", VisualHint: "Purple curve above linear", Duration: Duration(3 * time.Second), CodeRef: "merge_sort: split, sort, merge"},
						{Cue: "draw-quadratic", Narration: "O(n^2) is quadratic - grows explosively. Nested loops: 1000 items = 1 million operations!", VisualHint: "Red steep curve", Duration: Duration(3 * time.Second), CodeRef: "for i: for j: compare(i,j)"},
						{Cue: "scaling-demo", Narration: "Drag the slider to see real numbers. At n=100, O(n^2) is 10,000 ops. At n=1000, it is 1 million.", VisualHint: "Operations table updates", Duration: Duration(4 * time.Second), CodeRef: "scaling: watch numbers grow"},
						{Cue: "time-space", Narration: "Remember: you can trade space for time. Hash tables use O(n) space to achieve O(1) lookup.", VisualHint: "Time-space reference table", Duration: Duration(3 * time.Second), CodeRef: "tradeoffs: memoization, caching"},
						{Cue: "compare", Narration: "At scale, algorithm choice matters more than hardware. O(n^2) on a supercomputer loses to O(n log n) on a phone.", VisualHint: "Large n comparison", Duration: Duration(3 * time.Second), CodeRef: "insight: choose wisely"},
					},
				},
			},
			Tutorials: []Tutorial{
